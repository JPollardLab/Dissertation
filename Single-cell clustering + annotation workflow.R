...
##Cluster 4 highly expresses RAMP2,GNG11,VWF,EMCN and PLVAP relative to other clusters. 
#Points to possible endothelial cell identity. 
Cluster_4<-FindMarkers(Wu,ident.1 = 4)
head(Cluster_4,n=5)
VlnPlot(Wu,features = c("RAMP2","GNG11","PLVAP","VWF","EMCN"))
FeaturePlot(Wu,features = c("RAMP2","GNG11","PLVAP","VWF","EMCN"))

##Cluster 2 highly expresses LUM, CTSK, DCN,SFRP2,COL1A2 and FAP compared to other clusters. 
#May point to cluster being of 'Cancer associated fibroblast (CAF)' identity.
Cluster_2<-FindMarkers(Wu,ident.1=2)
head(Cluster_2,n=5)
VlnPlot(Wu,features = c("DCN","LUM","CTSK","COL1A2","SFRP2","FAP"))
FeaturePlot(Wu,features = c("DCN","LUM","CTSK","COL1A2","SFRP2","FAP"))

##Cluster 6 highly expresses IGFBP7, MYL9, NDUFA4L2 and ACTA2, whilst being negative for 
#Fibroblast associated markers like DCN or SFRP2. Likely identity is of 'smooth muscle cell'. 
Cluster_6<-FindMarkers(Wu,ident.1 = 6)
head(Cluster_6,n=5)
Cluster_6vs2<-FindMarkers(Wu,ident.1 = 6,ident.2 = 2)
head(Cluster_6vs2,n=5)
Cluster_6v4<-FindMarkers(Wu,ident.1 = 6,ident.2 = 4)
head(Cluster_6v4,n=5)
VlnPlot(Wu,features = c("IGFBP7","MYL9","NDUFA4L2","ACTA2","SFRP2","DCN"))
FeaturePlot(Wu,features = c("IGFBP7","MYL9","NDUFA4L2","ACTA2","SFRP2","DCN"))
...
VlnPlot(Wu,features = c("FOLR2","MS4A6A","CD68"))
FeaturePlot(Wu,features = c("FOLR2","MS4A6A","CD68"))

##Clusters 5,14 and 17 are enriched for MZB1,DERL3,JCHAIN. Clusters 5 and 14 highly express immunoglobulin-related genes.
#This points to their plasmablast identity. Clusters 5 and 14 do not substantially differ from one another
#therefore both labelled as plasmablast. Cluster 17 displays particularly high expression of GZMB,
...
VlnPlot(Wu,features=c("MZB1","JCHAIN","IGHG1","IGHG4","DERL3","PLAC8","LILRA4","GZMB","IRF8"))
FeaturePlot(Wu,features=c("MZB1","JCHAIN","IGHG1","IGHG4","DERL3","PLAC8","LILRA4","GZMB","IRF8"))
...
#Cluster 0 shows a rich population of IL7R rich T-cells, with low CD4, CD8 and GZMB expression. 
...
Cluster_0vsall$Gene_Name<-rownames(Cluster_0vsall)
VlnPlot(Wu,features = c("IL7R","CD3D"))
FeaturePlot(Wu,features = c("IL7R","CD3D"))
...
VlnPlot(Wu,features = c("NKG7","CCL4","CCL5","GZMB","CD3D"))
FeaturePlot(Wu,features = c("NKG7","CCL4","CCL5","GZMB","CD3D"))
...
VlnPlot(Wu,features = c("CD3D","IGKV1-5"))
FeaturePlot(Wu,features = c("CD3D","IGKV1-5"))
...
VlnPlot(Wu,features = c("STMN1","MKI67","TYMS","CD3D"))
FeaturePlot(Wu,features = c("STMN1","MKI67","TYMS","CD3D"))
...
Cluster_19vsall$Gene_Name<-rownames(Cluster_19vsall)
VlnPlot(Wu,features = c("CD3D","KLRC2","KLRD1","NKG7","GZMB"))
FeaturePlot(Wu,features = c("CD3D","GZMB","KLRC2","KLRD1","NKG7"))
...
#Cluster 7, has shown to express CRYAB,SAA1, exclusively relative to other epithelial clusters.
#Thus cluster assigned as 'CRYAB,SAA1 positive epithelial'. 
...
FeaturePlot(Wu,features = c("CRYAB","SAA1","EPCAM"))
VlnPlot(Wu,features = c("CRYAB","SAA1","EPCAM"))

#Cluster 8, has shown to express AGR3 and MLPH, exclusively relative to other epithelial clusters
#Largest expression was shown for AGR3 (strong marker for breast cancer cells) and MLPH, thus cluster assigned as 'Cancer epithelial cells'. 
Cluster_8<-FindMarkers(Wu,ident.1=8)
head(Cluster_8,n=5)
Cluster_8vsall<-FindMarkers(Wu,ident.1 = 8,ident.2 = c(7,13,15,16))
head(Cluster_8vsall,n=5)
VlnPlot(Wu,features = c("AGR3","MLPH","EPCAM"))
FeaturePlot(Wu,features = c("AGR3","MLPH","EPCAM"))

#Cluster 13 expresses high levels of S100A8,S100A9 relative to other epithelial clusters. 
#Assigned as 'S100A8 positive epithelial cells'.
...
VlnPlot(Wu,features = c("S100A8","S100A9","EPCAM"))
FeaturePlot(Wu,features = c("S100A8","S100A9","EPCAM"))

#Cluster 15 expresses substantially higher levels of CD24, compared to other epithelial clusters and other clusters in general.
#Very high expression of CD24 may point to cancer epithelial identity, thus cluster assigned as 'Cancer epithelial cells' 
...
VlnPlot(Wu,features = c("FDCSP","EPCAM","CD24"))
FeaturePlot(Wu,features=c("FDCSP","EPCAM","CD24"))

#Cluster 16 expresses high levels of SCGB1B2P and MUCL1 relative to other epithelial cluster
...
VlnPlot(Wu,features=c("SCGB1B2P","MUCL1","EPCAM"))
FeaturePlot(Wu,features=c("SCGB1B2P","MUCL1","EPCAM"))

###Assigning cluster names accordingly. 
#Naming clusters according to cell type identified
New.cluster.ids1<-c("IL7R rich T-cells","NKG7 rich T-cells","CAFs","FOLR2 negative macrophages",
                    "Endothelial cells","Plasmablasts","Smooth muscle cells","CRYAB,SAA1 positive epithelial cells",
                    "Cancer epithelial cells","IGKV1-5 positive, T-cell rich","B cells","FOLR2 positive macrophages",
                    "Cycling cells, T-cell rich","S100A8 positive epithelial cells","Plasmablasts",
                    "Cancer epithelial cells","SCGB1B2P positive epithelial cells",
                    "pDC","Myoepithelial cells","NK cell positive, T-cell positive")
...
#Combine both clusters labelled as 'Plasma cells' 
Wu$renamedCellTypes = as.factor(Idents(Wu))
